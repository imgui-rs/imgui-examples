    ...
    fn show_textures(&self, ui: &Ui) {
        ui.window("Hello textures")
            ...
            .build(|| {
                ...
                if let Some(sipi) = &self.sipi {
                    ui.text("Say hello to sipi-4-2-07.png");
                    sipi.show(ui);
                }

                // Example of using custom textures on a button
                if let Some(peppers) = &self.sipi {
                    ui.text("The Pepper buttons");

                    {
                        ...
                        draw_list
                            .add_image(peppers.texture_id, ui.item_rect_min(), ui.item_rect_max())
                            .col(tint)
                            .build();
                    }

                    {
                        ...
                        let draw_list = ui.get_window_draw_list();
                        draw_list
                            .add_image_quad(peppers.texture_id, tl, tr, br, bl)
                            .build();
                    }
...
impl SipiPng {
    ...
    {
        let image =
            image::io::Reader::new(Cursor::new(include_bytes!("../resources/sipi-4-2-07.png")))
                .with_guessed_format()
                .unwrap()
                .decode()
